...
CostLinearLoadManagerModel::~CostLinearLoadManagerModel() {

}

CostLinearLoadManagerModel::CostLinearLoadManagerModel(
		const nSharmaParameters& p, IOdictionary dict, PerformanceModel* pm,
		CommGraph* cG) :
		LinearLoadManagerModel(p, dict, pm, cG) {
...
void Foam::CostLinearLoadManagerModel::PostLoadMigrationUpdate() {
	...
	if (redistributeTimes.size() < 10 && lastTotalcellsTransfered > 0) {
		...
		float redistTime = float(
				allInfo_.find("local_redistribute")->second->totalTime());
		...
		if (redistributeTimes.size() > 2)
			linearRegression(cellsTransfered, redistributeTimes,
...
bool Foam::CostLinearLoadManagerModel::Balance(labelList& newD) {
	...
	for (std::vector<std::pair<int, labelList> >::iterator it =
			rawDecompCandidates.begin(); it != rawDecompCandidates.end();
			++it) {

		decompCandidates.push_back(
				new decompDetails((*it).second, (*it).first));
	...
	if (!doBalance)
		Info << "\nBalance-episode-log: Very few cells required to move"
				<< endl;
	...
	if (doBalance) {
		...
		if (!doBalance) {
			Info << "\nBalance-episode-log: Zero cells is non-sense, aborting"
					<< endl;
			return doBalance;
		}
		...
		//TODO: HACK
		int expected_iterations = PARAMS_.simulation_iterations /*- nSharma::iterationCount()*/;
		expected_iterations = expected_iterations > 0 ? expected_iterations : 1;

		for (std::vector<decompDetails*>::iterator it =
				decompCandidates.begin(); it != decompCandidates.end(); ++it) {
			...
			float Tstar__;
			MPI_Allreduce(&Tstar, &Tstar__, 1, MPI_FLOAT, MPI_MAX,
					MPI_COMM_WORLD);
			D.Tstar = Tstar__;
			...
			if (D.Tmigrt <= 0) {
				if (redistributeTimes.size() > 0) {
					D.Tmigrt = redistributeTimes[0] / cellsTransfered[0]
							* D.totalcellsTransfered;
				} else
					D.Tmigrt = 0.0;
			}

			float Tmigrt__;
			MPI_Allreduce(&(D.Tmigrt), &Tmigrt__, 1, MPI_FLOAT, MPI_MAX,
					MPI_COMM_WORLD);
			D.Tmigrt = Tmigrt__;

			Info << "Decomposition-candidate: " << " = T:" << T << " - Tstar:"
					<< D.Tstar << " + Tmigrt:" << D.Tmigrt
		...
		doBalance = best != NULL
				&& (T
						- (best->Tstar
								+ (best->Tmigrt / float(expected_iterations))))
						> (PARAMS_.minimal_gain * T);

		Pout << "previousCells " << previousCells << " newCells "
				<< best->totalReceived + best->count[Pstream::myProcNo()]
				<< " T " << T << " Tstar " << best->Tstar << " Tmigrt "
				<< best->Tmigrt << " totalSend " << best->totalSend
				<< " totalReceived " << best->totalReceived << endl;

		if (Pstream::master()) {

			Info << "\nBalance-episode-log: " << doBalance << " = T:" << T
					<< " - Tstar:" << best->Tstar << " + Tmigrt:"
					<< best->Tmigrt << " / expected_iterations:"
					<< expected_iterations << " > " << "PARAMS_.minimal_gain: "
					<< PARAMS_.minimal_gain << " * T:" << T << endl;
		...
		if (doBalance) {
			Info << "system-totalcellsTransfered " << best->totalcellsTransfered
					<< endl;
			lastTotalcellsTransfered = best->totalcellsTransfered;
		}

		newD = best->decomposition;
		//chose the best parameter even if we dont balance
		//because we are only filtering parameters here, not choosing the best impact
		partitioner->decompChosen(best->index);
